<template>
  <div>
    ...
    <div style="padding: 10px 0">
      <el-input style="width: 200px" placeholder="请输入工号" suffix-icon="el-icon-search" v-model="username"></el-input>
      ...
      <el-form label-width="80px" size="small">
        ...
        <el-form-item label="部门">
          <el-select clearable v-model="form.deptId" placeholder="请选择部门" style="width: 100%">
            <el-option v-for="item in depts" :key="item.id" :label="item.name" :value="item.id" :disabled="item.disabled"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  async load(){
     ...
     await this.request.get("/role").then(res => {
        this.roles = res.data
      })

     await this.request.get("/dept/list").then(res => {
    ...
    async handleStatusChange(row) {
      ...
      // console.log(index)
     await this.request.post("/user",row)
     await this.load()
    },
    //导出
    exp(){
